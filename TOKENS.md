...
## Color — Nightshade (the lush OKLCH system, 2026-06-18)

Generated by `themeEngine.js` from an OKLCH recipe; `petals-tokens.css` carries
the matching STATIC defaults (first-paint truth). The two are in lockstep —
change the recipe, regenerate the statics. Full design language (surfaces, text
ramp, accent/source roles, state grammar, the recipe) lives in
`obsidian/Petals.fm/Brand/folium-color-system.md`.

Two beauty principles fixed the old "drab" set:
1. **Surfaces carry violet life** — h282 / c0.022 (belladonna undertone), not
   dead grey. The field harmonizes with the purple anchor.
2. **Accents have a focal hierarchy** — purple is the fixed anchor (highest
   chroma, the brand); the others each earn their own L+C so the set reads as
   a considered hierarchy, never a flat row.

### Surfaces (h282 c0.022)

| Token | Hex | Role |
|-------|-----|------|
| `--bg` | #0b0b15 | violet-black floor |
| `--surface` | #151621 | panel |
| `--surface-up` | #21222d | raised / hover |
| `--surface-head` | #070711 | header recess |
| `--surface-base` | #070711 | base plate strip |
| `--groove` / `--control` | #070711 / #21222d | recessed / raised control |

### Text ramp (four tiers)

| Token | Hex | Use |
|-------|-----|-----|
| `--text` | #d6d0c9 | primary — names, hero values, active labels |
| `--text-value` | #aba39a | value words at rest (warm ivory) |
| `--text-dim` | #7e7974 | secondary, units, idle controls |
| `--text-muted` | #61636a | 7px chrome labels, inactive (cool steel) |

### Accents — the GUNDAM set, focal hierarchy

| Token | Hex | Name | Role |
|-------|-----|------|------|
| `--accent` | #7d6fd8 | Purple | brand anchor · LFO1 · active/focus |
| `--accent2` | #368cc6 | Blue | LFO2 |
| `--accent3` | #c26169 | Rose | ENV2 · danger neighbour |
| `--accent4` | #409f76 | Jade | ENV1 · lock · info |
| `--accent5` | #5a7ca1 | Steel | MTS-ESP · Motion 2 (quietest) |
| `--accent-env1` | #409f76 | Jade | mod source ENV 1 (= accent4) |
| `--accent-env2` | #c26169 | Rose | mod source ENV 2 (= accent3) |
| `--active` | #9a92e5 | Phosphor | "you are touching this" — purple glow |

Each accent has a `-dim` variant (L −0.13, C ×0.62); some have `-glow`.
One green only (jade). No gold, no sage-as-accent5.

### State grammar — one ladder for every control

Semantic role tokens. Every interactive element rides these, never a raw
color per control. A new instrument inherits the grammar by consuming them.

| Token | = | Meaning |
|-------|---|---------|
| `--state-idle` | `--text-muted` | at rest, quiet chrome |
| `--state-hover` | `--text` | pointer over it — affordance lift |
| `--state-active` | `--active` | engaged / on / playing |
| `--state-selected` | `--active` | chosen of a set |
| `--state-locked` | `--accent4` | constrained to a scale |
| `--state-disabled` | `--alpha-disabled` | applied via opacity, not hue |

Bare-text ladder: `--text-muted → --text → --active`.
Boxed controls add a surface on active: `--groove → --control`.

### Modulation source colors (six sources)

A source's color is its IDENTITY across every surface (mod bar, dial ring,
graph trace). Destinations are NOT colored.

| Source | Token | Hue |
|--------|-------|-----|
| LFO 1 | `--mod1` (`--accent`) | purple |
| LFO 2 | `--mod2` (`--accent2`) | blue |
| ENV 1 | `--mod-env1` (`--accent-env1`) | jade |
| ENV 2 | `--mod-env2` (`--accent-env2`) | rose |
| Motion 1 | `--mod-mot1` (`--accent`) | purple |
| Motion 2 | `--mod-mot2` (`--accent5`) | steel |

`--lock` = `--accent4` (jade) · `--ext` = `--accent5` (steel, MTS-ESP).

`--danger`: #c54d49

Kinari (light) mirrors this entire system with deep botanical inks on raw
silk; same hues + roles, inverted light axis. See themeEngine.js / the obsidian spec.
